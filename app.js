        ...
        html += `
            <div class="community-group">
                ...
                <div class="children-grid">
                    ${communityChildren.map(child => renderChildCard(child)).join('')}
                </div>
            </div>
        `
    }
...
function renderChildCard(child) {
    ...
    const donateButton = isFinished || isReserved
    ...
    return `
        <div class="child-card ${isFinished ? 'finished' : ''}" data-child-id="${child.id}">
            ${statusBadge}
            <img src="${child.poza_url}" alt="${child.nume}" class="child-image" />
            <div class="child-info">
                ...
                <div class="child-actions">
                    <button class="btn btn-secondary" onclick="openLetter('${child.id}')">
                        Scrisoare
                    </button>
                    ${donateButton}
                </div>
...
async function donate(childId) {
    ...
    try {
        ...
        alert(`Eroare: ${error.message}. Verifică că funcțiile Edge sunt configurate corect.`)
    }
}
...
function searchChildren(query) {
    ...
    const filtered = allChildren.filter(child => {
        return child.nume.toLowerCase().includes(lowerQuery) ||
